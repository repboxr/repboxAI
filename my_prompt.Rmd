...
example = function(project_dir) {
  ...
  debug_steps = "map_reg_run_debug1"
  ...
  rai_debug_rerun_map_reg_run(project_dir)
...
rai_debug_rerun_map_reg_run = function(project_dir, ignore_map_versions=NULL, just_map_versions=NULL, debugid = "debug1",doc_type = "art", to_v0=TRUE, overwrite=FALSE) {
  restore.point("rai_rerun_pru_map_reg_run")

  prod_id = "map_reg_run"
  rme=rai_load_rme(project_dir, doc_type)

  issues = rme$evals[issue_types]
  if (!is.null(ignore_map_versions)) {
    issues = lapply(issues, function(issue) {
      issue[!issue$map_version %in% ignore_map_versions,]
    })
  }
  if (!is.null(just_map_versions)) {
    issues = lapply(issues, function(issue) {
      issue[issue$map_version %in% just_map_versions,]
    })
  }

  pru_grid = bind_rows(lapply(issues, function(issue) {
    issue %>%
      select(map_version, tabid) %>%
      unique()
  })) %>% unique()

  fp_dir = file.path(project_dir, "fp", paste0("prod_", doc_type))

  pru_grid$ver_dir = fp_ver_id_to_ver_dir(fp_dir,prod_id, pru_grid$map_version)



  pru_entry = pru_grid[1,]
  org_ver_dir = pru_entry$ver_dir
  org_verid = pru_entry$map_version
  tabid = pru_entry$tabid
  org_proc_id = fp_ver_dir_to_proc_id(pru_entry$ver_dir)


  proc_id = paste0(org_proc_id, "_", debugid,"_", tabid)
  pru =
    rai_pru_base(project_dir,prod_id = prod_id, doc_type=doc_type, overwrite=overwrite,tpl_file = paste(prod_id,"_",debugid),proc_id = proc_id, to_v0 = to_v0) %>%
    rai_pru_add_doc() %>%
    rai_pru_add_run_do(in_context = FALSE)
...
rai_rerun_issue_types_map_reg_run = function() {
  c("multicol_reg_plausibility")
}
...
rai_pru_add_debug_doc_map_reg_run = function(pru) {
...
```
# END OF FILE: proc_debug_rerun.R
...
The file `proc_debug_rerun.R` shall contain code that allows to rerun the `map_reg_run` prompts for specific tables where the consistency checks in `repboxRegmap` suggest problems. I will modify the prompt and in particular add as media the document created by the functions in `rr_table.R` from repboxReport. Output is a new mapping for the particular table. The original `map_reg_run` directly generates output for all tables. So the corrected mapping must be merged with the original results, only the corrected tables will be changed. Currently I would generate a separate `pru` object and FuzzyProduction proc_id for each debugged table. But can you check whether somewhere there is already a better existing mechanism, e.g. to automatically combine all debug table runs. Note that the media will be different for each table. Just look at the code and give me advide how to best structure it.
