...
repbox_classify_prods = function() {
  prods_define(
    ...
    prod_define("reg_classify", list(
      tabid = schema_str("The table ID as stated in the list of regressions above."),
      regid = schema_int("The regression ID as stated in the field 'regid' in the JSON array of regressions shown above."),
      short_descr =  schema_str("A short description of what the regression analyzes based on the information in the article.", allow_null = FALSE),
      is_did_reg = schema_bool("true if the regression performs a difference-in-difference (DID) analysis."),
      is_rdd_reg = schema_bool("true if the regression performs a regression discontinuity design (RDD) analysis."),
      is_iv_reg =  schema_bool("true if it is an instrumental varibale regression"),
...
I would like to integrate the option to have a semi manual pipeline to generate a product version. The pru run should first generate a complete prompt, where all documents are included as text and store it as "full_prompt.txt" in the version directory. Then the user can copy it to a LLM chat interface, get the result and store it in a file "ai_resp.md" in the version folder. Then there shall be a general function that checks for manual results and updates the pru object for those versions and created the prod_df.Rds etc. 

Discuss how to best implement so that it fits nicely in the current pipeline with relatively little changes. Maybe already generate a file "rai_manual.R" that contains main helper functions.
